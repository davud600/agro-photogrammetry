...
class PlotBuilder:
    def __init__(self, num_of_indices_to_show: int):
    ...
    def create_plot(self):
        self.fig = self.plt.figure(
            figsize=(16, 10), dpi=120, facecolor=(0.8, 0.8, 0.8))
        ...
        self.plt.subplots_adjust(
            left=0.02, bottom=0.075, right=0.98, top=0.925, wspace=0.05, hspace=0.25)

    def create_subplot(self, row_index: int, col_index: int, index, index_type: str, index_label: str):
        colors = [(0, 'red'), (0.5, 'yellow'), (1, 'green')]
        custom_cmap = LinearSegmentedColormap.from_list(
            'custom_cmap', colors, N=256)

        self.indices[index_type] = index
        self.subplots[index_type] = self.fig.add_subplot(
            self.gs[row_index, col_index], facecolor=(0.9, 0.9, 0.9))
        ...
        self.subplots[index_type].set_title(
            f"{index_type} - {index_label}", fontsize=10)
        ...
        if flatten_index_len % 2 != 0:
            ...
            median = (int(flatten_index[int(flatten_index_len / 2)]) +
                      int(flatten_index[int(flatten_index_len / 2) + 1])) / 2
        ...
        self.subplots[index_type].text(
            0, -0.06, f'Mean: {mean}', ha='left', transform=self.subplots[index_type].transAxes, fontsize=8)
        self.subplots[index_type].text(
            0, -0.12, f'Median: {median}', ha='left', transform=self.subplots[index_type].transAxes, fontsize=8)
        ...
        cbar = self.fig.colorbar(
            img, ax=self.subplots[index_type], orientation='vertical')
        cbar.ax.set_position([cbar.ax.get_position().x0, self.subplots[index_type].get_position(
        ).y0, cbar.ax.get_position().width, self.subplots[index_type].get_position().height])

        # Connect event handlers for zoom and pan
        self.fig.canvas.mpl_connect(
            'scroll_event', partial(self.on_scroll, index_type))
        self.fig.canvas.mpl_connect(
            'button_press_event', partial(self.on_press, index_type))
        self.fig.canvas.mpl_connect('button_release_event', self.on_release)
        self.fig.canvas.mpl_connect(
            'motion_notify_event', partial(self.on_motion, index_type))
        self.rect_selector = RectangleSelector(self.subplots[index_type], onselect=None, useblit=True, button=[
                                               0], minspanx=5, minspany=5, spancoords='pixels', interactive=True)

    def on_scroll(self, index_type: str, event):
    ...
    def zoom(self, x, y, scale_factor: float, index_type: str):
    ...
    def zoom(self, x, y, scale_factor: float, index_type: str):
        ...
        self.subplots[index_type].set_xlim(
            [x - new_width * (1 - relx), x + new_width * relx])
        self.subplots[index_type].set_ylim(
            [y - new_height * (1 - rely), y + new_height * rely])
        self.fig.canvas.draw()

    def on_press(self, index_type: str, event):
        if event.inaxes != self.subplots[index_type]:
            return
        self.press = event.x, event.y, self.subplots[index_type].get_xlim(
        ), self.subplots[index_type].get_ylim()

    def on_motion(self, index_type: str, event):
